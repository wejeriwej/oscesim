...
function startTimer() {
  ...
  timerDuration = timerSlider.value * 60 * 1000;
  timerEnd = Date.now() + timerDuration;
  timerInterval = setInterval(updateTimer, 1000);
  startButton.disabled = true;
  pauseButton.disabled = false;
...
function updateTimer() {
  ...
  if (remainingTime < 0) {
    ...
    const progress = (1 - remainingTime / timerDuration) * 100;
    timerBar.style.width = progress + '%';
    minutesDisplay.textContent = remainingMinutes.toString();
    secondsDisplay.textContent = remainingSeconds.toString().padStart(2, '0');
    if (remainingTime <= 60000) {
      timerBar.style.backgroundColor = '#ff9900';
    }
  }
}
...
function startTimer(duration) {
  const now = Date.now();
  const then = now + (duration * 60 * 1000);
...
function unpauseTimer(duration) {
  const secondsLeft = parseInt(durationValue.textContent) * 60;
  const now = Date.now();
  ...
  countdown = setInterval(() => {
    ...
    durationValue.textContent = `${range.value}:00`;
    });
    
    startBtn.addEventListener('click', () => {
    if (isPaused) {
    unpauseTimer(parseInt(durationValue.textContent));
    } else {
    startTimer(parseInt(durationValue.textContent));
    }
    });
...
/*
document.getElementById("mp4_src").src = "WIN_20230328_13_59_37_Pro.mp4";
...
// Function to toggle full screen overlay
...
overlayContent.addEventListener("click", function (event) {
  event.stopPropagation(); // Prevent full screen toggle when content is clicked
});
...
function startFunction() { 
  ...
  if (document.getElementById("myVideo").requestFullscreen) {document.getElementById("myVideo").requestFullscreen();} 
  else if (document.getElementById("myVideo").webkitRequestFullscreen) { /* Safari */ document.getElementById("myVideo").webkitRequestFullscreen();} 
  else if (document.getElementById("myVideo").msRequestFullscreen) { /* IE11 */document.getElementById("myVideo").msRequestFullscreen();}
  
 /* var video = document.getElementById('myVideo');
  var image = document.querySelector('listeninggif');
  
  video.addEventListener('fullscreenchange', function() {
    if (document.fullscreenElement) {
      // Video is in full screen mode, show the image
      image.style.display = 'block';
    } else {
      // Video is not in full screen mode, hide the image
  ...
  */
  ...
  recognition.start();  document.getElementById('listeninggif').style.display = 'unset'; document.getElementById('enter3times').style.display = 'unset';
...
//this one moves it to the first question
...
pauseBtn.addEventListener("click", function() {
  if (pauseBtn.innerHTML === "Pause consultation") {
    ...
    recognition.stop(); document.getElementById('listeninggif').style.display = 'none'; document.getElementById('enter3times').style.display = 'none';
  } else {
    timer = setInterval(updateTimeLeft, 1000);
...
silenceTimer = setTimeout(function() {
  ...
  recognition.stop();
  //newQuestion();
  document.getElementById('listeninggif').style.display = 'none'; document.getElementById('enter3times').style.display = 'none';
...
//if(noteContent===''){
  //readOutLoud("Sorry we didn't catch your question, can you repeat what you said")
    //}
   // else{}
...
//  }
    ...
    if(noteContent.includes("name") && noteContent.includes("age")) {
      ...
      document.getElementById("mp4_src").src = "WIN_20230328_13_59_49_Pro.mp4"; document.getElementById("myVideo").load();
...
let generalhistorycounter = 0;
     ...
     if (radiatex === true){
      radiatetest.text('')
      radiatetest1.text('● Whether the pain radiated to anywhere')
      generalhistorycounter++
     }
     ...
     if (ratepainx === true){
      ratepaintest.text('')
      ratepaintest1.text('● The severity of the pain')
      generalhistorycounter++
     }
     ...
     if (makeitworsex === true){
      ...
      makeitworsetest1.text('● Whether or not anything makes the pain worse')
      generalhistorycounter++
     }
     ...
     if (makeitbetterx === true){
      ...
      makeitbettertest1.text('● Whether or not anything makes the pain better')
      generalhistorycounter++
     }
     ...
     if (walkdistancex === true){
      ...
      walkdistancetest1.text('● How far they can walk')
      generalhistorycounter++
     }
     ...
     if (ideasx === true){
      ideastest.text('')
      ideastest1.text("● The patient's ideas")
      generalhistorycounter++
     }
     ...
     if (concernsx === true){
      concernstest.text('')
      concernstest1.text("● The patient's concerns")
      generalhistorycounter++
     }
     ...
     if (expectationsx === true){
      ...
      expectationstest1.text("● The patient's expectations")
      generalhistorycounter++
     }
     ...
     if (familyhistoryx === true){
      ...
      familyhistorytest1.text('● Family history')
      generalhistorycounter++
     }
     ...
     if (smokingx === true){
      smokingtest.text('')
      smokingtest1.text('● Whether the patient smoked')
      generalhistorycounter++
     }
     ...
     if (alcoholx === true){
      alcoholtest.text('')
      alcoholtest1.text('● Whether the patient drank any alcohol')
      generalhistorycounter++
...
recognition_examinations.onstart = function() {
  recordinginstructionsforexaminations.text('Microphone is activated. Try speaking into the microphone. Then double click the Submit Question button.');
}
 
recognition_examinations.onspeechend = function() {
  recordinginstructionsforexaminations.text('DOUBLE CLICK THE SUBMIT BUTTON. (Once you get the response, press the Microphone button to ask another question)');
}
 
recognition_examinations.onerror = function(event) {
  if(event.error == 'no-speech') {
    recordinginstructionsforexaminations.text('No speech was detected. Try again.');  
  };
}
...
if(!noteexaminations.length) {
  recordinginstructionsforexaminations.text('Could not save empty note. Please add a message to your note.');
...
recognition_differentials.onstart = function() {
  recordinginstructionsfordifferentials.text('Microphone is activated. Try speaking into the microphone. Then double click the Submit Question button.');
}
 
recognition_differentials.onspeechend = function() {
  recordinginstructionsfordifferentials.text('DOUBLE CLICK THE SUBMIT BUTTON. (Once you get the response, press the Microphone button to ask another question)');
}
 
recognition_differentials.onerror = function(event) {
  if(event.error == 'no-speech') {
    recordinginstructionsfordifferentials.text('No speech was detected. Try again.');  
  };
}
...
if(!noteDifferentials.length) {
  recordinginstructionsfordifferentials.text('Could not save empty note. Please add a message to your note.');
}
else {
  // Save note to localStorage.
  // The key is the dateTime with seconds, the value is the content of the note.
...
recognition_investigations.onstart = function() {
  recordinginstructionsforinvestigations.text('Microphone is activated. Try speaking into the microphone. Then double click the Submit Question button.');
}
 
recognition_investigations.onspeechend = function() {
  recordinginstructionsforinvestigations.text('DOUBLE CLICK THE SUBMIT BUTTON. (Once you get the response, press the Microphone button to ask another question)');
}
 
recognition_investigations.onerror = function(event) {
  if(event.error == 'no-speech') {
    recordinginstructionsforinvestigations.text('No speech was detected. Try again.');  
  };
}
...
if(!noteinvestigations.length) {
  recordinginstructionsforinvestigations.text('Could not save empty note. Please add a message to your note.');
}
else {
  // Save note to localStorage.
  // The key is the dateTime with seconds, the value is the content of the note.
